...
class AAstar:
    def __init__(self, size, start, end, grid):
        ...
        self.closed = None # used to update H-new values
        self.check_neighbors(self.start)
        
        # declare global current g(goal)
        # after each cell expansion, update the closed set to have H-new  = g(goal) - g(s)
        ...
        while True:
            ...
            self.map = np.copy(self.blocked) # creates current maze vision
    ...
    def solve(self):
        while True:
            if len(self.heap) == 0:
                ...
                break
            min = heapq.heappop(self.heap)[2] # retrieve next best cell
            self.moves += 1
            self.current = min
            self.closed.add(min)
            if self.current.x == self.end.x and self.current.y == self.end.y:
                # updates current presumed shortest path length
                self.goal = self.current.g
                break
            self.map[self.current.x, self.current.y] = 2 # min[0]
            self.add_neighbors(self.current)
        
    def initialize(self):
        # update closed list h-new values
        self.closed = set()
